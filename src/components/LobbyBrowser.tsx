  ...
  const handleJoinLobby = async (lobbyAddress: string, gameId?: number) => {
    ...
    try {
      ...
      if (!targetGameId) {
        ...
        const gameAccount = await (
          program as any
        ).account.pirateGame.fetchNullable(new PublicKey(lobbyAddress));
        if (!gameAccount) {
    ...
    <div className="bg-slate-900/50 backdrop-blur-md rounded-2xl border-2 border-neon-cyan/30 p-6 w-full max-w-4xl mx-auto shadow-2xl">
      <div className="flex items-center justify-between mb-8">
        <div>
          ...
          <p className="text-gray-400 text-sm">
            Find an active arena or spawn your own
          </p>
        </div>
        <button
        ...
        <label className="flex items-center gap-3 cursor-pointer">
          ...
          <div>
            ...
            <p className="text-xs text-gray-400">
              Use session key to hide your identity
            </p>
          </div>
        </label>
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-yellow-400/70 text-sm">
                Connect your wallet to join battles
              </p>
            </div>
          </div>
          <button
      ...
      {isLoading && lobbies.length === 0 ? (
        <div className="flex flex-col items-center py-12">
          ...
          <p className="text-neon-cyan font-mono animate-pulse">
            SCANNING SEAS...
          </p>
        </div>
      ) : lobbies.length === 0 ? (
        <div className="text-center py-12 bg-slate-800/30 rounded-xl border border-dashed border-slate-700">
          <p className="text-gray-500 mb-4">
            No active battles found in your sector.
          </p>
          <button
            onClick={() => setShowCreateModal(true)}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {lobbies
            ...
            .map((lobby: any) => {
              ...
              const statusKey = lobby.status
                ? Object.keys(lobby.status)[0]?.toLowerCase()
                : "unknown";
              ...
              const isUserInGame =
                userWalletPk && lobby.players?.includes(userWalletPk);
              const isUserGameWaiting =
                isUserInGame && isWaiting && lobby.playerCount === 1;

              // Check if user is the game creator (authority) and can start the game
              const isGameCreator =
                userWalletPk && lobby.authority === userWalletPk;
              const canStartGame =
                isGameCreator && isWaiting && lobby.playerCount >= 2;
                ...
                >
                  <div className="flex justify-between items-start mb-4">
                    ...
                    <div className="text-right">
                      ...
                      <div
                        className={`text-xs px-2 py-0.5 rounded border font-bold uppercase tracking-wider ${
                          isUserGameWaiting
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex -space-x-2">
                      ...
                      {arenaFilter === "shadow" && statusKey === "waiting" ? (
                        ...
                        <>
                          {Array.from({ length: Math.max(0, maxPlayers) }).map(
                            (_, idx) => (
                              <div
                                key={idx}
                        ...
                        <>
                          ...
                          {Array.from({
                            length: Math.max(0, maxPlayers - lobby.playerCount),
                          }).map((_, idx) => (
                            <div
                              key={`empty-${idx}`}
                    ...
                    <div className="flex gap-2">
                      {canStartGame && (
                        <button
                          onClick={() =>
                            handleStartGame(lobby.address, lobby.gameId)
                          }
                      ...
                      <button
                        onClick={() =>
                          handleJoinLobby(lobby.address, lobby.gameId)
                        }
                        disabled={
                          !!joiningLobby ||
                          !isJoinable ||
      ...
      {showCreateModal && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/80 backdrop-blur-sm p-4">
          <div className="bg-slate-900 border-2 border-neon-gold/50 rounded-2xl p-8 max-w-md w-full shadow-2xl animate-in zoom-in-95">
            <h3 className="text-2xl font-black text-neon-gold mb-4 uppercase">
              SPAWN ARENA
            </h3>
            <div className="space-y-4 mb-8">
              <div>
                <label className="block text-xs font-mono text-gray-500 mb-2">
                  GAME LOBBY SEED (NUMBER)
                </label>
                <input
                  type="number"
                  value={newGameId}
