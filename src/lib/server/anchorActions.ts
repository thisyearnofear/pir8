...
export async function getAnchorClient(): Promise<{
  ...
  try {
    ...
    const programId = SOLANA_CONFIG.PROGRAM_ID
      ...
      : PROGRAM_ID;
    console.log(
      "[Anchor Client] Loading program for read-only access:",
      programId.toString(),
    );
...
export async function fetchGlobalGameState(gameId: number = 0): Promise<any> {
  ...
  try {
    ...
    console.log(
      `[Anchor Client] Derived gamePDA: ${gamePDA.toBase58()} for gameId: ${gameId}`,
    );

    try {
      const rawState = await (program as any).account.pirateGame.fetch(gamePDA);
      console.log(
        `[Anchor Client] Successfully fetched state for game ${gameId}`,
      );
      ...
      if (
        ...
        console.log(
          `[Anchor Client] Game ${gameId} account not found (PDA: ${gamePDA.toBase58()})`,
        );
        return null;
      }
      console.error(
        `[Anchor Client] Error fetching game state for ${gameId} (PDA: ${gamePDA.toBase58()}):`,
        error,
      );
      throw error;
    }
    ...
    console.error(
      `[Anchor Client] Critical error in fetchGlobalGameState for ${gameId}:`,
      outerError,
    );
    throw outerError;
  }
...
export async function initializeGlobalGame(): Promise<string> {
  throw new Error(
    "DEPRECATED: Server should not execute user transactions. Use client-side wallet signing.",
  );
}
