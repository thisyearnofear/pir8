...
export class AgentMatchmaker {
    ...
    while (availableAgents.length >= 2) {
      ...
      for (let i = 0; i < availableAgents.length && match.length < 4; i++) {
        ...
        const eloDiff = Math.abs(
          candidate.agent.eloRating - baseAgent.agent.eloRating,
        );

        // Check if within preferred skill range
        if (
          eloDiff <= baseAgent.preferences.skillRange &&
          eloDiff < bestEloDiff
        ) {
          // Check avoid list
          if (
            !baseAgent.preferences.avoidOpponents?.includes(
              candidate.agent.publicKey,
            )
          ) {
            bestOpponentIndex = i;
            bestEloDiff = eloDiff;
      ...
      while (match.length < 4 && availableAgents.length > 0) {
        const avgElo =
          match.reduce((sum, m) => sum + m.agent.eloRating, 0) / match.length;
  ...
  async findOpponents(
    ...
    const onlineAgents = this.agentRegistry
      .getOnlineAgents()
  ...
  async createBalancedLobby(agents: AgentProfile[]): Promise<GameLobby> {
    ...
    const avgElo =
      agents.reduce((sum, agent) => sum + agent.eloRating, 0) / agents.length;
    ...
    const lobby: GameLobby = {
      ...
      skillLevel,
      gameType: "casual",
      entryFee: GAME_CONFIG.ENTRY_FEE * 0.5, // Lower fee for casual games
