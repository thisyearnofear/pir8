...
export class AgentRegistry {
  ...
  async registerAgent(profile: Partial<AgentProfile>): Promise<AgentProfile> {
    ...
    const performanceMetrics: AgentPerformanceMetrics = {
      ...
      territoryControlPatterns: Array(10)
        .fill(null)
        .map(() => Array(10).fill(0)),
      eloRating: 1200,
    ...
    // Persist to local storage for hackathon demo
    this.persistAgentData(profile.publicKey!, agentProfile, performanceMetrics);

    return agentProfile;
  }

  /**
   * Persist agent data to localStorage (temporary until on-chain storage is ready)
   */
  private persistAgentData(
    publicKey: string,
  ...
  private loadAgentData(
    publicKey: string,
  ): { profile: AgentProfile; metrics: AgentPerformanceMetrics } | null {
    try {
      const data = localStorage.getItem(`pir8_agent_${publicKey}`);
      if (data) {
        ...
        return {
          profile: {
            ...
            lastSeen: new Date(parsed.profile.lastSeen),
          },
          metrics: {
            ...parsed.metrics,
            eloHistory: parsed.metrics.eloHistory.map((h: any) => ({
              ...h,
              date: new Date(h.date),
            })),
            performanceOverTime: parsed.metrics.performanceOverTime.map(
              (p: any) => ({ ...p, date: new Date(p.date) }),
            ),
          },
        };
      }
    } catch {
      console.warn("Failed to load agent data from localStorage");
    }
    return null;
  }
  ...
  getAgent(publicKey: string): AgentProfile | undefined {
    ...
    const cached = this.agents.get(publicKey);
    if (cached) return cached;

    // Fall back to persistent storage
    const loaded = this.loadAgentData(publicKey);
    if (loaded) {
      this.agents.set(publicKey, loaded.profile);
      this.performance.set(publicKey, loaded.metrics);
   ...
   */
  getAgentsBySkillLevel(
    skillLevel: AgentProfile["skillLevel"],
  ): AgentProfile[] {
    return this.getAllAgents().filter(
      (agent) => agent.skillLevel === skillLevel,
    );
  }
  ...
  getOnlineAgents(): AgentProfile[] {
    ...
    return this.getAllAgents().filter(
      (agent) => agent.isOnline && agent.lastSeen > fiveMinutesAgo,
    );
  }
  ...
  async updatePerformance(
    ...
    performance.averageScore =
      (performance.averageScore * (performance.totalGames - 1) +
        gameResult.score) /
      performance.totalGames;

    performance.averageDecisionTime =
      (performance.averageDecisionTime * (performance.totalGames - 1) +
        gameResult.decisionTime) /
      performance.totalGames;

    // Update ELO rating
    const opponentPerformance = this.performance.get(
      gameResult.opponentPublicKey,
    );
    ...
    if (performance.performanceOverTime.length >= 5) {
      const scores = performance.performanceOverTime
        .slice(-10)
        .map((p) => p.score);
  ...
  findOpponents(agentPublicKey: string, count: number = 3): AgentProfile[] {
    ...
    const onlineAgents = this.getOnlineAgents().filter(
      (a) => a.publicKey !== agentPublicKey,
    );
