...
async function checkGamePlayerCount(gameId: string, expectedCount: number): Promise<boolean> {
    try {
        ...
        const activePlayers = gameAccount.players
            .filter((player: any) => player.key.toString() !== '11111111111111111111111111111111')
            .length;

        console.log(`[Game Recovery] Game ${gameId} has ${activePlayers} players (expected ${expectedCount})`);
...
export function useGameIdRecovery(options: GameIdRecoveryOptions) {
    ...
    const { gameState, setGameState } = usePirateGameState();
    ...
    const attemptRecovery = useCallback(async () => {
        ...
        setTimeout(async () => {
            try {
                ...
                const currentState = usePirateGameState.getState().gameState;
                ...
                if (currentPlayerCount >= expectedPlayerCount) {
                    console.log(`[Game Recovery] Game already has ${currentPlayerCount} players, no recovery needed`);
                    return;
                }

                console.log(`[Game Recovery] Current game ${currentGameId} has ${currentPlayerCount} players, expected ${expectedPlayerCount}`);
                ...
                if (correctGameId && correctGameId !== currentGameId) {
                    console.log(`[Game Recovery] Switching from ${currentGameId} to ${correctGameId}`);
                    ...
                    setTimeout(() => {
                        console.log('[Game Recovery] Reloading page to fully sync with correct game');
